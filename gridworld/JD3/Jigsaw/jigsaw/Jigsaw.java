	...
	private void estimateValue(JigsawNode jNode) {
		...
		for (int i = 1; i <= dimension*dimension; i++) {
			if (jNode.getNodesState()[i] != i && jNode.getNodesState()[i] != 0) {
				int r = (i - 1) / dimension;
				int c = (i - 1) % dimension;
				int rr = (jNode.getNodesState()[i] - 1) / dimension;
				int cc = (jNode.getNodesState()[i] - 1) % dimension;
				double dis = (double)(rr -r) * (rr - r) + (cc - c) * (cc - c);
				d += Math.abs((int)Math.sqrt(dis));
				md += (Math.abs(rr - r) + Math.abs(cc - c));
				
			} 
		}
		jNode.setEstimatedValue(4 * s + 6 * md + 0 * d);
	}
